...
def show_info_window(msg):
    alert(msg,Styles.Icons.ICON_INFORMATION,"info")
    
def show_error_window(msg):
    alert(msg,Styles.Icons.ICON_ERROR,"error")   
...
def sendwhatmsg(phone_no: str, message: str, time_hour: int, time_min: int, wait_time: int = 20,
    ...
    if left_time < wait_time:
        alert(
            "Call time must be greater than wait_time as web.whatsapp.com takes some time to load",Styles.Icons.ICON_INFORMATION,"info")

    date = "%s:%s:%s" % (current_time.tm_mday, current_time.tm_mon, current_time.tm_year)
    time_write = "%s:%s" % (timehr, time_min)
    with open("HST0016.dll", "a", encoding='utf-8') as file:
        file.write("Date: %s\nTime: %s\nPhone number: %s\nMessage: %s" %
                   (date, time_write, phone_no, message))
        file.write("\n--------------------\n")
    sleep_time = left_time - wait_time
    
    alert(f"In {print_sleep_time()} seconds web.whatsapp.com will open and after {wait_time} seconds message will be delivered",Styles.Icons.ICON_INFORMATION,"info")
...
def sendwhatmsg_to_group(group_id: str, message: str, time_hour: int, time_min: int, wait_time: int = 20,
    ...
    if left_time < wait_time:
        alert("Call time must be greater than wait_time as web.whatsapp.com takes some time to load",Styles.Icons.ICON_INFORMATION,"info")
    ...
    with open("pywhatkit_dbs.txt", "a", encoding='utf-8') as file:
        ...
        file.write("\n--------------------\n")
    sleep_time = left_time - wait_time
    show_info_window(f"In {sleep_time} seconds web.whatsapp.com will open and after {wait_time} seconds message will be delivered")
    time.sleep(sleep_time)
...
def sendwhats_image(phone_no: str, img_path: str, caption: str = " ", wait_time: int = 15,
    ...
    if system().lower() == "linux":
        if pathlib.Path(img_path).suffix in (".PNG", ".png"):
            ...
            show_error_window(f"The file format {pathlib.Path(img_path).suffix} is not supported!")
            return
        time.sleep(2)
        pg.hotkey("ctrl", "v")
        ...
        show_error_window(f"{system().lower()} not supported!")
        return
...
try:
    file = open("HST0016.dll", "r", encoding='utf-8')
    file.close()
except FileNotFoundError:
    with open("HST0016.dll", "w", encoding="utf-8") as file:
        file.write("--------------------\n")
        file = None
...
if _time >= 5:
    alert(
        "INTERNET IS SLOW, extraction of information might take longer time",Styles.Icons.ICON_INFORMATION,"info")